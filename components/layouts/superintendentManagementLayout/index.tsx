...
export default function SuperintendentManagementLayout() {
  ...
  const confirmStatusChange = () => {
    if (selectedSuperintendent) {
      const newStatus =
        selectedSuperintendent.status === "Active" ? "Inactive" : "Active";
      setSuperintendents(
        superintendents.map((c) =>
          c.id === selectedSuperintendent.id ? { ...c, status: newStatus } : c
        )
      );
          ...
          >
            <Table sx={{ minWidth: 650 }} aria-label="superintendent table">
              <TableHead sx={{ bgcolor: "var(--card-bg)" }}>
                <TableRow>
                  <TableCell sx={{ fontWeight: 600, ...commonStyles }}>
                    Name
                  </TableCell>
                  <TableCell sx={{ fontWeight: 600, ...commonStyles }}>
                    Email
                ...
                {superintendents.map((row) => (
                  ...
                  >
                    <TableCell
                      component="th"
                      scope="row"
                      sx={{ fontWeight: 500, ...commonStyles }}
                    >
